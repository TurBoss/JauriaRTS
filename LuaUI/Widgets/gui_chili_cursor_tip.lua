...
local function getUnitTooltip(uID)
	...
	local health = select(1,spGetUnitHealth(uID))
	local maxHealth = select(2,spGetUnitHealth(uID))
	...
	tooltip = tooltip ..'\n'.. formatresource("Life: ", health) ..'/' .. maxHealth .. '\n' .. 'space+click for info'
...
local function getFeatureTooltip(fID)
    ...
    if fName then tooltip = firstToUpper(fName) end
	return tooltip
end
-----------------------------------
