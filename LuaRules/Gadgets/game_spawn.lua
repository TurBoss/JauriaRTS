...
local function SpawnStartUnit(teamID)
	...
	if (startUnit and startUnit ~= "") then
		...
		newUnitID = 1000 + teamID
		local unitID = Spring.CreateUnit(startUnit, x, y, z, facing, teamID, false, true, newUnitID)
	end

	-- set start resources, either from mod options or custom team keys
	local teamOptions = select(7, Spring.GetTeamInfo(teamID))
	local m = teamOptions.startmetal  or modOptions.startmetal  or 2000
	local e = teamOptions.startenergy or modOptions.startenergy or 10000
	local ms = 10000
	local es = 10000
	...
	if (e and tonumber(e) ~= 0) then
		...
		--   must be done after the start unit is spawned,
		--   otherwise the starting resources are lost!
		Spring.SetTeamResource(teamID, "es", tonumber(es))
